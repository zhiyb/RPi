...
int main(void)
{
	...
	class gString *str = new gString(0, point(-107), angle(), 0x66CCFF, "HelloWD!", 4.5);
	/*class sphereFrame *sph = */new sphereFrame(str, point(-30, 50, 0), 30, 0x66CCFF);
	new sphereFrame(new sphere(str, point(240, 50, 0), 30, 0x00FF00), point(), 20, 0xFF0000);
	...
	while (true) {
		...
		tstr.resize(8);
		str->setString(tstr);
		gAxis->setA(angle(0.5, gAxis->a().y() + 0.02, 0));
		if (gAxis->a().y() > 2 * PI)
			gAxis->setA(0.5);
